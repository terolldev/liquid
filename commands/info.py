...
class InfoCommand(commands.Cog):
    ...
    async def virable (self, inter, virable=None):
      if virable == "{timestamp}":
        ...
        embed=disnake.Embed(title="Virable All", description="`{timestamp}`, `{author}",
        color=0x2e2f33, timestamp=datetime.datetime.now())
    ...
    @bot.slash_command(description='Информация о пользователе | User information', options=[
        disnake.Option(
            "пользователь", description="Выберите пользователя!", type=disnake.OptionType.user, required=False,
        ),],)
    async def user(self, inter, пользователь=None):
